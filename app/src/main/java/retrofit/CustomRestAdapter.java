...
public class CustomRestAdapter {
  ...
  private CustomRestAdapter(Endpoint server,
        ...
        return rxSupport.createRequestObservable(new RxSupport.Invoker() {
          ...
          public Observable call(Throwable throwable) {
            if (throwable instanceof RetrofitError
                ...
                && methodInfo.requestMethod.equals("GET")) {
              return rxSupport.createRequestObservable(new RxSupport.Invoker() {
                @Override
                public ResponseWrapper invoke(RequestInterceptor requestInterceptor) {
                  return (ResponseWrapper) invokeRequest(addStaleHeader(requestInterceptor),
                      methodInfo,
                      args);
                }
              });
  ...
  private RequestInterceptor addStaleHeader(RequestInterceptor requestInterceptor) {
    return new RequestInterceptor() {
      @Override
      public void intercept(RequestFacade request) {
        request.addHeader("Cache-Control", "public, only-if-cached, max-stale=" + 60 * 60 * 24);
  ...
  public static class Builder {
     ...
     *     Executor on which any {@link Callback} methods will be invoked. If
     *     this argument is {@code null} then callback methods will be run on the same thread as
     *     the
     *     HTTP client.
     */
     ...
     */
    public CustomRestAdapter build() {
      if (endpoint == null) {
        throw new IllegalArgumentException("Endpoint may not be null.");
      }
      ensureSaneDefaults();
      return new CustomRestAdapter(endpoint,
          clientProvider,
          httpExecutor,
          callbackExecutor,
          requestInterceptor,
